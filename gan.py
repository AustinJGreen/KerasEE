...
def train(epochs, batch_size, worldCount, versionName = None):
    curDir = os.getcwd()
    ganDir = utils.check_or_create_local_path("gan")
    ...
    dAccRealSummary = tf.Summary()
    dAccRealSummary.value.add(tag='d_acc_real', simple_value=None)
    dAccFakeSummary = tf.Summary()
    dAccFakeSummary.value.add(tag='d_acc_fake', simple_value=None)
    dLossSummary = tf.Summary()
    dLossSummary.value.add(tag='d_loss', simple_value=None)
    dLossRealSummary = tf.Summary()
    dLossRealSummary.value.add(tag='d_loss_real', simple_value=None)
    dLossFakeSummary = tf.Summary()
    dLossFakeSummary.value.add(tag='d_loss_fake', simple_value=None)
    ...
    for epoch in range(epochs):
        ...
        for minibatchIndex in range(numberOfBatches):
            ...
            dAccRealSummary.value[0].simple_value = d_realAcc
            tbWriter.add_summary(dAccRealSummary, (epoch * numberOfBatches) + minibatchIndex)

            # Write loss for real labels
            dLossRealSummary.value[0].simple_value = d_realLoss
            tbWriter.add_summary(dLossRealSummary, (epoch * numberOfBatches) + minibatchIndex)
            ...
            dAccFakeSummary.value[0].simple_value = d_fakeAcc
            tbWriter.add_summary(dAccFakeSummary, (epoch * numberOfBatches) + minibatchIndex)

            # Write loss for fake labels
            dLossFakeSummary.value[0].simple_value = d_fakeLoss
            tbWriter.add_summary(dLossFakeSummary, (epoch * numberOfBatches) + minibatchIndex)

            # Calculate average loss and accuracy
            d_avgAcc = (d_realAcc + d_fakeAcc) / 2.0
            d_avgLoss = (d_realLoss + d_fakeLoss) / 2.0

            # Write average accuracy for real and fake labels
            dAccSummary.value[0].simple_value = d_avgAcc
            tbWriter.add_summary(dAccSummary, (epoch * numberOfBatches) + minibatchIndex)

            # Write average loss for real and fake labels
            dLossSummary.value[0].simple_value = d_avgLoss
            ...
            print("epoch [%d/%d] :: batch [%d/%d] :: disAcc = %.1f%% :: disLoss = %f :: genLoss = %f" % (epoch, epochs, minibatchIndex, numberOfBatches, d_avgAcc * 100, d_avgLoss, g_loss))
